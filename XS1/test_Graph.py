...
class TestGraph(TestCase):
    ...
    def test_count_c(self):
        ...
        assert 2/3 == graph.count_c()
        graph = Graph.read_graph_from_file('skipjacka_lin.txt')
        assert 8 / 15 == graph.count_c()
        graph = Graph.read_graph_from_file('skipjacka_diff.txt')
        assert 8 / 15 == graph.count_c()
